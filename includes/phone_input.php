...
if (!empty($_pf_value)) {
    if ($_pf_value[0] === '+') {
        // BUG FIX: this used to be preg_match('/^(\+\d{1,4})(\d+)$/', ...), which
        // is greedy — \d{1,4} always grabs 4 digits when 4+ remain, so +63 (a
        // 2-digit code) was being mis-split as "+6394" / "54356681" instead of
        // "+63" / "9454356681". That silently ate 2 real digits off the front of
        // the local number every time a stored value got re-parsed, which is why
        // the number shrank by 2 digits on every single save. Matching against
        // the actual known dial codes (longest first, so e.g. +886 isn't cut
        // short by a false +8 match) fixes it for every country in the list.
        $_pf_digits = substr($_pf_value, 1);
        $_pf_codes  = array_unique(array_column($_pf_countries, 1));
        usort($_pf_codes, fn($a, $b) => strlen($b) - strlen($a));
        foreach ($_pf_codes as $_pf_code) {
            $_pf_code_digits = ltrim($_pf_code, '+');
            if (str_starts_with($_pf_digits, $_pf_code_digits)) {
                $_pf_dialcode = $_pf_code;
                $_pf_local    = substr($_pf_digits, strlen($_pf_code_digits));
                break;
            }
        }
