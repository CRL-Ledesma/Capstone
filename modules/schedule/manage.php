...
// $current_user_id and $current_user_name are normally set by auth.php
...
foreach ($days_list as $d) {
    $chk = $conn->prepare("SELECT COUNT(*) as c FROM schedules WHERE day_of_week = ?");
    $chk->execute([$d]);
    $count = (int)$chk->fetch(PDO::FETCH_ASSOC)['c'];
    ...
    if ($count > 1) {
        ...
        $conn->prepare("
            ...
            AND id NOT IN (
                SELECT id FROM (
                    SELECT MAX(id) as id FROM schedules WHERE day_of_week = ?
                ) as t
            )
        ")->execute([$d, $d]);
    } elseif ($count == 0) {
