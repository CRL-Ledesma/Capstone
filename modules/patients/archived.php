...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['perma_delete_id'])) {
    ...
    if ($did && $confirmed) {
        ...
        if ($row) {
            $pname = $row['n'];
            $conn->prepare("DELETE FROM bills           WHERE patient_id = ?")->execute([$did]);
            $conn->prepare("DELETE FROM dental_records  WHERE patient_id = ?")->execute([$did]);
            $conn->prepare("DELETE FROM appointments    WHERE patient_id = ?")->execute([$did]);
            $conn->prepare("DELETE FROM patients        WHERE id         = ?")->execute([$did]);
